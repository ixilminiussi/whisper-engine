...
// https://github.com/SergeyMakeev/SlotMap/blob/main/slot_map/slot_map.h
...
// #define SLOT_MAP_ASSERT(expression) (void)((!!(expression)) || (_onAssertionFailed(#expression, __FILE__, (unsigned
// int)(__LINE__)), 0))
...
template <class T, size_t Alignment = std::max(alignof(T), alignof(void *))> struct Allocator
{
  public:
    ...
    template <typename U> Allocator(Allocator<U, Alignment> const & /* other */) noexcept
    {
    }
    ...
    pointer allocate(size_type n, [[maybe_unused]] void const *hint = 0)
    {
...
bool operator==(Allocator<T1, Alignment> const & /*lhs*/, Allocator<T2, Alignment> const & /*rhs*/) noexcept
{
    return true;
}
...
/*
Even though slot map keys are technically typeless (uint64_t), we artificially add a new type to get extra compiler
checks.
...
template <typename T> struct slot_map_key64
{
    ...
    static inline index_t toIndex(slot_map_key64 key) noexcept
    {
    ...
    static inline version_t increaseVersion(version_t version) noexcept
    {
        return (version + 1);
    }

    inline tag_t get_tag() const noexcept
    {
        return static_cast<tag_t>((raw & kHandleTagMask) >> kHandleTagShift);
    }
    ...
    explicit slot_map_key64(id_type raw) noexcept : raw(raw)
    {
    }
    slot_map_key64(slot_map_key64 const &) noexcept = default;
    slot_map_key64 &operator=(slot_map_key64 const &) noexcept = default;
    slot_map_key64(slot_map_key64 &&) noexcept = default;
    slot_map_key64 &operator=(slot_map_key64 &&) noexcept = default;

    bool operator==(slot_map_key64 const &other) const noexcept
    {
...
template <typename T> struct slot_map_key32
{
    ...
    static inline index_t toIndex(slot_map_key32 key) noexcept
    {
        return static_cast<index_t>(key.raw & kIndexMask);
    }
    ...
    explicit slot_map_key32(id_type raw) noexcept : raw(raw)
    {
    }
    slot_map_key32(slot_map_key32 const &) noexcept = default;
    slot_map_key32 &operator=(slot_map_key32 const &) noexcept = default;
    slot_map_key32(slot_map_key32 &&) noexcept = default;
    slot_map_key32 &operator=(slot_map_key32 &&) noexcept = default;

    bool operator==(slot_map_key32 const &other) const noexcept
    {
        return raw == other.raw;
    }
    bool operator<(slot_map_key32 const &other) const noexcept
...
/*
  A slot map is a high-performance associative container with persistent unique keys to access stored values. Upon
  insertion, a key is returned that can be used to later access or remove the values. Insertion, removal, and access are
  all guaranteed to take O(1) time (best, worst, and average case) Great for storing collections of objects that need
  stable, safe references but have no clear ownership.

  The difference between a std::unordered_map and a slot map is that the slot map generates and returns the key when
  inserting a value. A key is always unique and will only refer to the value that was inserted.
...
*/
template <typename T, typename TKeyType = slot_map_key64<T>, size_t PAGESIZE = 4096, size_t MINFREEINDICES = 64>
class slot_map
{
  public:
    using key = TKeyType;
    ...
    /*
        ...
        Page size in elements. When all page items(slots) are disabled we automatically release that page to save
       memory. The page-based allocator also helps with pointers' stability and prevents memory spikes on reallocation.

        4096 seems like a reasonable number of items per page.
        I.e., for simple uint32_t type, it will take 16Kb of RAM per page = good compromise between CPU cache
       utilization, allocation granularity, and memory overhead from deactivated slots.
    */
    ...
    /*
        ...
        But since we only use 16-bits for version numbers, there is a possibility that the version counter will wrap
       around, and a new item will get the same key as a removed item.
        ...
        To prevent version overflow from happening too often, we need to ensure that we don't reuse the same slot too
       often. So we do not reuse indices as long as their number is below a certain threshold.
    */
  ...
  private:
    ...
    struct Page
    {
        ...
        Page() noexcept : rawMemory(nullptr), values(nullptr), meta(nullptr), numInactiveSlots(0), numUsedElements(0)
        {
        }
        ...
        Page(Page &&other) noexcept
            : rawMemory(nullptr), values(nullptr), meta(nullptr), numInactiveSlots(0), numUsedElements(0)
        {
        ...
        void allocate()
        {
            ...
            /*
              ...
              Passing a size which is not an integral multiple of alignment or an alignment which is not valid or not
              supported by the implementation causes the function to fail and return a null pointer (C11, as published,
              specified undefined behavior in this case, this was corrected by DR 460)
            */
    ...
    static inline size_type align(size_type cursor, size_type alignment) noexcept
    {
        return (cursor + (alignment - 1)) & ~(alignment - 1);
    }
    static inline bool isPointerAligned(void const *cursor, size_t alignment) noexcept
    {
    ...
    T const *getImpl(key k) const noexcept
    {
        ...
        Meta const &m = getMetaByAddr(addr);
        ...
        ValueStorage const &v = getValueByAddr(addr);
        SLOT_MAP_ASSERT(isPointerAligned(&v, alignof(T)));
        T const *value = reinterpret_cast<T const *>(&v);
        return value;
    }
    ...
    index_t appendElement()
    {
        ...
        index_t index =
            static_cast<index_t>(getIndexFromAddr(PageAddr{static_cast<size_type>(pages.size()) - 1, elementIndex}));
        return index;
    }
    ...
    Meta const &getMetaByAddrImpl(PageAddr addr) const noexcept
    {
        SLOT_MAP_ASSERT(addr.page < pages.size());
        Page const &page = pages[addr.page];
        SLOT_MAP_ASSERT(page.meta);
        SLOT_MAP_ASSERT(addr.index < kPageSize);
        return page.meta[addr.index];
    }

    ValueStorage const &getValueByAddrImpl(PageAddr addr) const noexcept
    {
        SLOT_MAP_ASSERT(addr.page < pages.size());
        Page const &page = pages[addr.page];
    ...
    Meta const &getMetaByAddr(PageAddr addr) const noexcept
    {
        return getMetaByAddrImpl(addr);
    }
    ValueStorage const &getValueByAddr(PageAddr addr) const noexcept
    {
        return getValueByAddrImpl(addr);
    }

    Meta &getMetaByAddr(PageAddr addr) noexcept
    {
        Meta const &constRes = getMetaByAddrImpl(addr);
        return const_cast<Meta &>(constRes);
    }

    ValueStorage &getValueByAddr(PageAddr addr) noexcept
    {
        ValueStorage const &constRes = getValueByAddrImpl(addr);
    ...
    bool isTombstone(size_type index) const noexcept
    {
        ...
        SLOT_MAP_ASSERT(addr.index < kPageSize);
        Meta const &meta = page.meta[addr.index];
    ...
    size_type getMaxValidIndex() const noexcept
    {
        return maxValidIndex;
    }

    void copyFrom(slot_map const &other)
    {
        reset();
        ...
        {
            Page const &otherPage = other.pages[pageIndex];
            if (otherPage.meta)
            {
                ...
                else
                {
                    for (size_type elementIndex = 0; elementIndex < p.numUsedElements; elementIndex++)
                    {
                        ...
                        ValueStorage const &otherVal = other.getValueByAddr(addr);
                        SLOT_MAP_ASSERT(isPointerAligned(&otherVal, alignof(T)));
                        T const *otherV = reinterpret_cast<T const *>(&otherVal);
        ...
        for (size_t pageIndex = 0; pageIndex < pages.size(); pageIndex++)
        {
            ...
            for (size_type elementIndex = 0; elementIndex < page.numUsedElements; elementIndex++)
            {
                ...
                if constexpr (!std::is_trivially_destructible<T>::value)
                {
                    ...
                    SLOT_MAP_ASSERT(isPointerAligned(&v, alignof(T)));
                    destruct(reinterpret_cast<T const *>(&v));
                }
                numItemsDestroyed++;
            }
        }
    ...
    template <bool VERSION_CHECK> EraseResult eraseImpl(key k)
    {
        ...
        if constexpr (!std::is_trivially_destructible<T>::value)
        {
            ValueStorage &v = getValueByAddr(addr);
            SLOT_MAP_ASSERT(isPointerAligned(&v, alignof(T)));
            T const *pv = reinterpret_cast<T const *>(&v);
            destruct(pv);
        }
        numItems--;
  ...
  public:
    slot_map() : numItems(0), maxValidIndex(0)
    {
    }
    ~slot_map()
    {
    ...
    */
    ...
    /*
      ...
      Automatically increases version for all the removed elements (the same as calling "erase()" for all existing
      elements)
    */
    void clear()
    {
    ...
    */
    T const *get(key k) const noexcept
    {
        return getImpl(k);
    }
    ...
    */
    ...
    /*
      Removes element (if such key exists) from the slot map, returning the value at the key if the key was not
      previously removed.
    */
    ...
    */
    ...
    // copy constructor
    slot_map(slot_map const &other) : numItems(0), maxValidIndex(0)
    {
        copyFrom(other);
    }

    // copy assignment
    slot_map &operator=(slot_map const &other)
    {
        copyFrom(other);
        return *this;
    }
    ...
    // move constructor
    slot_map(slot_map &&other) noexcept : numItems(other.numItems), maxValidIndex(other.maxValidIndex)
    {
    ...
    */
    Stats debug_stats() const noexcept
    {
        ...
        for (size_t pageIndex = 0; pageIndex < pages.size(); pageIndex++)
        {
            Page const &page = pages[pageIndex];
            if (page.meta == nullptr)
            {
  ...
  public:
    ...
    {
      private:
        using slot_map_ptr = std::conditional_t<IsConst, slot_map const *, slot_map *>;
        using value_ptr = std::conditional_t<IsConst, T const *, T *>;
        using value_ref = std::conditional_t<IsConst, T const &, T &>;
        using storage_ref = std::conditional_t<IsConst, ValueStorage const &, ValueStorage &>;
      ...
      public:
        explicit values_iterator_impl(slot_map_ptr _slotMap, size_type index) noexcept
            : slotMap(_slotMap), currentIndex(index)
        {
        }

        // Allow non-const to const conversion
        template <bool OtherIsConst, typename = std::enable_if_t<IsConst && !OtherIsConst>>
        values_iterator_impl(values_iterator_impl<OtherIsConst> const &other) noexcept
            : slotMap(other.slotMap), currentIndex(other.currentIndex)
        {
        }
        ...
        bool operator==(values_iterator_impl const &other) const noexcept
        {
    ...
    {
        return const_values_iterator(this, getMaxValidIndex() + static_cast<size_type>(1));
    }
    ...
    values_iterator end() noexcept
    {
        return values_iterator(this, getMaxValidIndex() + static_cast<size_type>(1));
    }
    ...
    template <bool IsConst> class kv_iterator_impl
    {
      public:
        ...
        template <typename TYPE> struct reference
        {
            ...
            explicit reference(TYPE *_ptr) : ptr(_ptr)
            {
            }

            reference(reference const & /*other*/) noexcept = default;
            reference(reference && /*other*/) noexcept = default;
            reference &operator=(reference const & /*other*/) noexcept = default;
            reference &operator=(reference && /*other*/) noexcept = default;

            void set(TYPE *_ptr) noexcept
            {
                ptr = _ptr;
            }
        ...
        using storage_ref = std::conditional_t<IsConst, ValueStorage const &, ValueStorage &>;
        using KeyValue =
            std::conditional_t<IsConst, std::pair<key, reference<T const> const>, std::pair<key, reference<T>>>;
      ...
      private:
        void updateTmpKV() const noexcept
        {
            ...
            PageAddr addr = slotMap->getAddrFromIndex(currentIndex);
            SLOT_MAP_ASSERT(slotMap->isActivePage(addr));
            Meta const &m = slotMap->getMetaByAddr(addr);
            storage_ref v = slotMap->getValueByAddr(addr);
            SLOT_MAP_ASSERT(slotMap->isPointerAligned(&v, alignof(T)));
            ...
            {
                const_cast<reference<T const> &>(tmpKv.second).set(value);
            }
            else
            {
      ...
      public:
        explicit kv_iterator_impl(slot_map_ptr _slotMap, size_type index) noexcept
            : slotMap(_slotMap), currentIndex(index),
              tmpKv(key::invalid(), std::conditional_t<IsConst, reference<T const>, reference<T>>(nullptr))
        {
        }

        // Allow non-const to const conversion
        template <bool OtherIsConst, typename = std::enable_if_t<IsConst && !OtherIsConst>>
        kv_iterator_impl(kv_iterator_impl<OtherIsConst> const &other) noexcept
            : slotMap(other.slotMap), currentIndex(other.currentIndex),
              tmpKv(key::invalid(), reference<T const>(nullptr))
        {
        }

        KeyValue const &operator*() const noexcept
        {
            updateTmpKV();
            return tmpKv;
        }
        ...
        bool operator==(kv_iterator_impl const &other) const noexcept
        {
            return slotMap == other.slotMap && currentIndex == other.currentIndex;
        }
        bool operator!=(kv_iterator_impl const &other) const noexcept
    ...
    template <bool IsConst> class items_impl
    {
      ...
      public:
        explicit items_impl(slot_map_ptr _slotMap) noexcept : slotMap(_slotMap)
        {
        }
        ...
        {
            return iterator_type(slotMap, slotMap->getMaxValidIndex() + static_cast<size_type>(1));
        }
    };
    ...
    MutableItems items() noexcept
    {
        return MutableItems(this);
    }
...
{
    size_t operator()(typename dod::slot_map_key64<T> const &key) const noexcept
    {
        return key.hash();
    }
};
...
{
    size_t operator()(typename dod::slot_map_key32<T> const &key) const noexcept
    {
        return key.hash();
    }
};
